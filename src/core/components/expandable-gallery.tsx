...
const ExpandableGallery: React.FC<ExpandableGalleryProps> = memo(({ images, className = '' }) => {
  ...
  return (
    <MotionConfig reducedMotion="user">
      <div className={className}>
        ...
        <div className="flex gap-2 h-128 w-full" style={{ transform: 'translateZ(0)' }}>
          {images.map((image, index) => (
            <motion.div
              key={index}
              layoutId={`gallery-item-${index}`}
              className="relative cursor-pointer overflow-hidden rounded-md will-change-[flex]"
              style={{ flex: 1 }}
...
ExpandableGallery.displayName = 'ExpandableGallery';
